...
# 中文翻译：获取运行时 Hook 状态的别名函数
# Get runtime hook state alias function
def get_hook_state(session_id: str, turn_idx: int, key: str) -> Optional[str]:
    return get_runtime_hook_value(session_id, turn_idx, key)

# 中文翻译：设置运行时 Hook 状态的别名函数
# Set runtime hook state alias function
...
# 中文翻译：删除运行时 Hook 状态的别名函数
# Delete runtime hook state alias function
def delete_hook_state(session_id: str, turn_idx: int, key: str) -> None:
    delete_runtime_hook_value(session_id, turn_idx, key)

# 中文翻译：根据当前回合 ID 清理历史残留 Hook 状态的别名函数
# Trim runtime hook states alias function based on current turn ID
