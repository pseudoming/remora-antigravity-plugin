...
describe("TestHookEntrypointRefactor", () => {
  it("test_system_exit_non_zero", () => {
    ...
    const decorated = hookEntrypoint({ decision: "allow" })(
      function dummyHook(_context: Record<string, unknown>): Record<string, unknown> {
...
describe("TestZombieDetectorInterception", () => {
  ...
  it("test_zombie_detected_pre_tool_use", () => {
    ...
    const readFileImpl = (p: any, _enc?: string): string | Buffer => {
      const path = String(p);
      if (path.includes("environ")) return Buffer.from("ANTIGRAVITY_AGENT=true\x00");
  ...
  it("test_manage_task_allowed_during_zombie_presence", () => {
    ...
    const result = zombieMain({ toolCall: { name: "manage_task", args: { Action: "list" } } });
