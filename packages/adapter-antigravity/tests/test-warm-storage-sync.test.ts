...
const { mockStreamForward, mockStreamReverse } = vi.hoisted(() => {
	const mockStreamForward =
		vi.fn<(startIdx?: number) => Generator<Record<string, any>>>();
	const mockStreamReverse =
		vi.fn<(limit?: number) => Generator<Record<string, any>>>();
...
vi.mock("../src/sidecar/scan-sessions", () => ({
	...
	extractSubagentReport: vi
		.fn()
		.mockReturnValue({ changedFiles: [], referencedFiles: [] }),
}));
...
vi.mock("@remora/core", () => {
	...
	function _resolve(args: any[]): {
		...
		return {
			db: new Database(TEST_DB_PATH, { timeout: 15000 }),
			closeAfter: true,
			a: args,
		};
	}
...
it("cursor resume", () => {
	...
	conn
		.prepare(
			"INSERT INTO watermarks (project_uuid, conversation_id, last_msg_id) VALUES ('proj1', 'conv1', 5)",
		)
		.run();
	for (let i = 1; i <= 5; i++) {
		conn
			.prepare(
				"INSERT INTO messages (id, conversation_id, line_number, content) VALUES (?, 'conv1', ?, ?)",
			)
			.run(i, i, `msg ${i}`);
	}
	conn.close();
	...
	try {
		const [keyContent, currentMsgId, lastMsgId] = readIncrementalLogs(
			db,
			session,
		);
		...
		const row = db.prepare("SELECT COUNT(*) as cnt FROM messages").get() as {
			cnt: number;
		};
...
it("proto role parsing", () => {
	...
	try {
		...
		const rows = db
			.prepare(
				"SELECT role, content FROM messages WHERE conversation_id='conv2' ORDER BY line_number ASC",
			)
