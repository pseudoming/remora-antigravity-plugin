...
const { mockReadFileSync, mockExistsSync, mockMkdirSync, mockWriteFileSync, realFs } = vi.hoisted(() => ({
...
describe("TestCleanWhitelist", () => {
  ...
  it("test_stale_pids_cleaned", () => {
    ...
    mockWriteFileSync.mockImplementation((p: string, data: string, encoding?: string) => {
      if (p === whitelistPath) {
