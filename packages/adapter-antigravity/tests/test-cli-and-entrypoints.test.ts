...
describe("tone_injector", () => {
	...
	it("strict mode injects STRICT TONE message", () => {
		...
		const res = toneInjector.main({
			transcriptPath: "/brain/c1/transcript.jsonl",
		});
...
describe("remora_recall", () => {
	...
	it("remora_recall_errors — no args, no db, no project uuid all exit(1)", () => {
		...
		expect(exitSpy).toHaveBeenCalledWith(1);
		expect(logSpy).toHaveBeenCalledWith(
			"Usage: remora-recall.ts <keyword> [project_uuid]",
		);
		...
		expect(() => remoraRecall()).toThrow("EXIT");
		expect(logSpy).toHaveBeenCalledWith(
			expect.stringContaining("无法获取项目标识"),
		);
	});
	...
	it("remora_recall_success — all three channels produce output and touch topics", () => {
		...
		expect(coreMocks.recallDecisionsByFts5Topic).toHaveBeenCalledWith(
			"p1",
			"",
			"cache",
		);
		expect(coreMocks.recallDecisionsByLike).toHaveBeenCalledWith(
			"p1",
			"",
			"cache",
		);
		expect(coreMocks.touchTopicsAccessedByRecall).toHaveBeenCalledWith(
			"p1",
			"",
			"cache",
		);
...
describe("remora_topic", () => {
	...
	it("remora_topic_errors — no action, no project uuid, no db all exit(1)", () => {
		...
		expect(() => remoraTopic()).toThrow("EXIT");
		expect(coreMocks.error).toHaveBeenCalledWith(
			expect.stringContaining("Project UUID is required"),
		);
	...
	it("remora_topic_success — new, switch, close actions work", () => {
		...
		process.argv = ["node", "remora-topic", "new", "-n", "topic_a"];
		remoraTopic();
		expect(coreMocks.createOrUpdateTopic).toHaveBeenCalledWith(
			"p1",
			"topic_a",
			"",
			"manual",
		...
		expect(logSpy).toHaveBeenCalledWith(
			expect.stringContaining("Created active topic"),
		);
		logSpy.mockClear();
		...
		expect(coreMocks.forceColdStartLatestSession).toHaveBeenCalled();
		expect(logSpy).toHaveBeenCalledWith(
			expect.stringContaining("Switched active topic"),
		);
		logSpy.mockClear();
		...
		expect(logSpy).toHaveBeenCalledWith(
			expect.stringContaining("closed in project"),
		);
	});
	...
	it("remora_topic_switch_no_name — switch without -n exits with error", () => {
		...
		expect(() => remoraTopic()).toThrow("EXIT");
		expect(coreMocks.error).toHaveBeenCalledWith(
			expect.stringContaining("Topic name"),
		);
	});
	...
	it("remora_topic_confirm_no_id — confirm without -d exits with error", () => {
		...
		expect(() => remoraTopic()).toThrow("EXIT");
		expect(coreMocks.error).toHaveBeenCalledWith(
			expect.stringContaining("Decision ID"),
		);
	});
	...
	it("remora_topic_confirm_failure — non-existent decision warns", () => {
		...
		expect(coreMocks.warn).toHaveBeenCalledWith(
			expect.stringContaining("No decision found"),
		);
	});
	...
	it("remora_topic_main_execution — end-to-end topic creation with uuid flag", () => {
		...
		expect(logSpy).toHaveBeenCalledWith(
			expect.stringContaining("Created active topic my_topic in project p2"),
		);
	});
	...
	it("remora_topic_confirm_sandbox_merge — confirm triggers sandbox merge with physical files", () => {
		...
		try {
			...
			expect(coreMocks.mergePhysicalFilesToTopic).toHaveBeenCalledWith(
				"p1",
				"t1",
				["my_file.py"],
			);
			...
			expect(logSpy).toHaveBeenCalledWith(
				expect.stringContaining("Integrated 1 physical changed files"),
			);
		} finally {
			fs.rmSync(tmpPath, { recursive: true, force: true });
		}
	...
	it("remora_topic_confirm_no_worktrees — confirm finds no sandbox worktrees", () => {
		...
		try {
			...
			expect(coreMocks.info).toHaveBeenCalledWith(
				expect.stringContaining("No active sandbox worktree found"),
			);
...
describe("snapshot_git", () => {
	...
	it("exception in getSnapshot returns empty injectSteps", () => {
		...
		try {
			...
			coreMocks.getSnapshot.mockImplementation(() => {
...
describe("subagent_monitor", () => {
	...
	it("no argv causes error exit", () => {
		...
		expect(exitSpy).toHaveBeenCalledWith(1);
		expect(logSpy).toHaveBeenCalledWith(
			expect.stringContaining("Missing conversation_id"),
		);
	});
	...
	it("stream error exits with status 1", () => {
		...
		try {
			...
			expect(exitSpy).toHaveBeenCalledWith(1);
			expect(logSpy).toHaveBeenCalledWith(
				expect.stringContaining("Failed to read db logs"),
			);
		} finally {
			fs.rmSync(tmpPath, { recursive: true, force: true });
		}
	...
	it.each([
		...
		try {
			...
			conversationMocks.mockInstance.streamStepsReverse = vi
				.fn()
				.mockReturnValue([{ type: stepType, content: "test" }]);
	...
	it("zombie detection with retry escalation", () => {
		...
		try {
			...
			conversationMocks.mockInstance.streamStepsReverse = vi
				.fn()
				.mockReturnValue([{ type: "RUN_COMMAND", content: "test" }]);
			conversationMocks.mockInstance.getDbMtime = vi.fn().mockReturnValue(1000);
			...
			const retryDir = path.join(
				tmpPath,
				".runtime",
				"remora_subagent_retries",
			);
			// Pre-create retry file with retry_count = 2
			fs.mkdirSync(retryDir, { recursive: true });
			fs.writeFileSync(
				path.join(retryDir, "conv_1.json"),
				JSON.stringify({ retry_count: 2 }),
			);
	...
	it("not zombie — clears retry file", () => {
		...
		try {
			...
			conversationMocks.mockInstance.streamStepsReverse = vi
				.fn()
				.mockReturnValue([{ type: "GREP_SEARCH", content: "test" }]);
			conversationMocks.mockInstance.getDbMtime = vi.fn().mockReturnValue(1000);
			coreMocks.judgeZombie.mockReturnValue([false, 120]);
