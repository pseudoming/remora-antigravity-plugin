...
function makeDb(dbPath: string, tablesWithRows: Record<string, Array<Array<string>>>): void {
  ...
  for (const [table, rows] of Object.entries(tablesWithRows)) {
    if (table === "session_state") {
      ...
      cur.exec("CREATE TABLE project_topics (id TEXT, title TEXT, status TEXT)");
    } else if (table === "topic_decisions") {
      cur.exec("CREATE TABLE topic_decisions (id TEXT, topic_id TEXT, decision TEXT)");
...
function humanSize(sizeBytes: number | null): string {
  ...
  for (const unit of ["B", "KB", "MB", "GB", "TB"]) {
    if (Math.abs(sizeBytes) < 1024.0) {
      return unit !== "B" ? `${sizeBytes.toFixed(1)} ${unit}` : `${sizeBytes} B`;
    }
    sizeBytes /= 1024.0;
  }
...
function getDbTables(dbPath: string): string[] {
  ...
  try {
    const rows = conn.prepare("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name").all() as Array<{ name: string }>;
...
describe("test_debug_env", () => {
  ...
  describe("TestFullReport", () => {
    it("test_full_report", () => {
      ...
      const installed = fs.existsSync(path.join(dataDir, ".runtime", "installed.flag"));
      ...
      outLines.push(`DB_SIZE:       ${dbSize !== null ? humanSize(dbSize) : "N/A (file not found)"}`);
  ...
  describe("TestDBAbsent", () => {
    it("test_db_file_absent_no_crash", () => {
      const dbPath = getDbPath();
      const dbSize = fs.existsSync(dbPath) ? humanSize(fs.statSync(dbPath).size) : "N/A (file not found)";
      const dbTables = getDbTables(dbPath);
      const counts = getDbTableCounts(dbPath);
