...
vi.mock("../src/sidecar/extract-decisions", async (importOriginal) => {
	const actual =
		await importOriginal<typeof import("../src/sidecar/extract-decisions")>();
...
import {
	...
	AgentApiError,
} from "../src/sidecar/extract-decisions";
...
describe("TestCalculateMd5", () => {
	it("test_known_content", () => {
		const tmpPath = realFs.value.mkdtempSync(
			path.join(os.tmpdir(), "remora_md5_"),
		);
		try {
	...
	it("test_binary_like_content", () => {
		...
		try {
			...
			const expected = crypto
				.createHash("md5")
				.update(Buffer.from([0x00, 0x01, 0x02, 0xff]))
				.digest("hex");
			expect(calculateMd5(f)).toBe(expected);
		} finally {
			realFs.value.rmSync(tmpPath, { recursive: true, force: true });
...
describe("TestIsSubagentSession", () => {
	it("returns true when getSubagentTypeByConvId returns typeName", () => {
		vi.mocked(subagentMod.getSubagentTypeByConvId).mockReturnValue(
			"Remora_Deep_Diver",
		);
				...
				yield {
					...
					tool_calls: [
						{ name: "write_to_file", args: { TargetFile: "/path/to/foo.py" } },
						{
							...
							args: { AbsolutePath: "/path/to/bar.js" },
						},
		...
		vi.spyOn(
			...
			(function* () {
				yield {
					step_index: 3,
					tool_calls: [
						{ name: "write_to_file", args: { TargetFile: "old.py" } },
					],
				};
				yield {
					step_index: 5,
					tool_calls: [
						{ name: "write_to_file", args: { TargetFile: "new.py" } },
					],
				};
			})(),
		);
...
describe("TestConsumeEventQueue", () => {
	...
	it("test_confirms_matching_decisions", () => {
		...
		vi.spyOn(Date, "now").mockReturnValue(0);
		consumeEventQueue(0);
		expect(coreMocks.confirmDecisionsByIds).toHaveBeenCalledWith(
			[10, 20],
			"p1",
			undefined,
		);
...
describe("TestPruneSidecarEvents", () => {
	...
	it("test_prunes_json_files", () => {
		...
		// Cleanup
		realFs.value.rmSync(path.join(testDataDir.value, "sidecar_data"), {
			recursive: true,
			force: true,
		});
	});
...
describe("TestGetOrCreateConversation", () => {
	...
	it("should recover and rollover when sidecar conversation db and pb files do not exist", () => {
		...
		try {
			...
			const newSavedConvId = realFs.value
				.readFileSync(markerFilePath, "utf-8")
				.trim();
			expect(newSavedConvId).toBe("new-conv-id-888");

			expect(agentapiMocks.createConversation).toHaveBeenCalled();
			expect(agentapiMocks.createConversation.mock.calls[0][0]).toContain(
				"test prompt",
			);
		} finally {
