...
const mocks = vi.hoisted(() => {
	...
	return {
		...
		isExemptedPath: vi
			.fn<[string], boolean>()
			.mockImplementation(realIsExemptedPath),
	};
});
...
const coreMocks = vi.hoisted(() => ({
	...
	enforcePromptLengthLimit: vi
		.fn()
		.mockReturnValue([false, null] as [
			boolean,
			{ prefix: string; message: string; action_tip: string } | null,
		]),
...
vi.mock("node:fs", async (importOriginal) => {
	...
	return {
		...
		readFileSync: vi.fn((p, enc) => {
			if (typeof p === "string" && p.includes("remora-rules.json"))
				return actual.readFileSync(realRulesPath, "utf-8");
...
describe("SafetyCheckWrapper", () => {
	...
	describe("getSubagentType", () => {
		...
		it("get_subagent_type_api_success", () => {
			...
			const res = main(
				makeCtx("view_file", { AbsolutePath: "/some/file.txt" }),
			);
		...
		it("get_subagent_type_fallback", () => {
			...
			const res = main(
				makeCtx("view_file", { AbsolutePath: "/path/to/log.jsonl" }),
			);
	...
	describe("invoke_subagent", () => {
		...
		it("allow with JIT injection", () => {
			...
			expect(steps[0]["ephemeralMessage"]).toContain(
				"REMORA COORDINATOR JIT INJECTION",
			);
	...
	describe("define_subagent override protection", () => {
		...
		it("allows Deep_Diver to write to non-parent_shared path", () => {
			mocks.getSubagentType.mockReturnValue("Remora_Deep_Diver");
			const result = main(
				makeCtx("multi_replace_file_content", {
	...
	describe("view_file", () => {
		it("sensitive suffixes deny in main context", () => {
			...
			expect(res["reason"]).toContain(
				"Access to large log files is restricted",
			);
		});
		...
		it("sensitive suffix without subagent deny", () => {
			...
			const ctx = makeCtx("view_file", {
				AbsolutePath: "/path/to/file.sqlite",
			});
			const res = main(ctx);
			expect(res["decision"]).toBe("deny");
			expect(res["reason"]).toContain(
	...
	describe("run_command", () => {
		it("rot feature deny in main context", () => {
			...
			const ctx = makeCtx("run_command", {
				CommandLine: "jq . /path/to/data.jsonl",
			});
			const res = main(ctx);
			expect(res["decision"]).toBe("deny");
		});
		...
		it("readonly deny non-allow command", () => {
			...
			const ctx = makeCtx("run_command", {
				CommandLine: "cat /path/to/data.jsonl",
			});
			const res = main(ctx);
			expect(res["decision"]).toBe("deny");
		...
		it("other category deny returns delegation", () => {
			...
			expect(res["decision"]).toBe("deny");
			expect(res["reason"]).toContain(
				"DELEGATION: Command verification failed",
			);
		});
	...
	describe("grep_search", () => {
		...
		it("sensitive dir subagent allows", () => {
			...
			const ctx = makeCtx("grep_search", {
				SearchPath: "/some/.system_generated/logs",
			});
			const res = main(ctx);
			expect(res["decision"]).toBe("allow");
		});
		...
		it("normal search allows", () => {
			...
			const ctx = makeCtx("grep_search", {
				SearchPath: "/path/to/normal_file.py",
			});
			const res = main(ctx);
			expect(res["decision"]).toBe("allow");
		});
	...
	describe("trim turn edge cases", () => {
		it("trim turn type error handled", () => {
			mocks.getSubagentType.mockReturnValue(null);
			mocks.cdallGetCurrentTurnIdx.mockReturnValue(
...
describe("GitCommitEscapeAndInheritWriteDeny", () => {
	...
	it("inherit write block for subagent write_to_file", () => {
		...
		const ctx = makeCtx("write_to_file", {
			TargetFile: "test.txt",
			CodeContent: "hello",
		});
		const res = main(ctx);
		expect(res["decision"]).toBe("deny");
		expect(res["reason"]).toContain(
			"Subagent execution in inherited workspace",
		);
	...
	it("blocks Remora_Merger from running non-git or restricted commands", () => {
		...
		const ctx2 = makeCtx("run_command", {
			CommandLine: "git checkout && npm run build",
		});
	...
	it("intercepts main agent git merge and recommends Remora_Merger subagent", () => {
		...
		coreMocks.inspectCommand.mockReturnValue([
			"deny",
			"any_category_other_than_build",
		]);

		const ctx = makeCtx("run_command", {
			CommandLine: "git merge feature-branch",
		});
		const res = main(ctx);
		expect(res["decision"]).toBe("deny");
...
describe("BehaviorRulesGuard", () => {
	...
	it("Duplicate spawn rate limiter", () => {
		...
		coreMocks.getHookState.mockImplementation(
			(convId: string, currentTurnIdx: number, key: string) => {
		...
		coreMocks.setHookState.mockImplementation(
			(convId: string, currentTurnIdx: number, key: string, val: string) => {
				if (key === "subagent_spawns") {
					localHistory = val;
				}
			},
	...
	describe("Symlink Escape Interception Test", () => {
		it("blocks view_file if target is a symlink pointing to sensitive path", () => {
			...
			const tempDir = pathReal.join(
				osReal.tmpdir(),
				`remora-symlink-test-${Date.now()}`,
			);
			...
			coreMocks.isRotSensitiveFile.mockImplementation((p) =>
				p.endsWith(".jsonl"),
			);
	...
	describe("send_message turn limit check (Phase 73)", () => {
		it("blocks send_message to ReadOnly extractor when turn limit exceeds 4", () => {
			mocks.getSubagentType.mockReturnValue(null);
			mocks.getSubagentTypeByConvId.mockReturnValue(
			...
			coreMocks.getHookState.mockImplementation(
				(id: string, turn: number, key: string) => {
					return localStore[key] || null;
				},
			);
			coreMocks.setHookState.mockImplementation(
				(id: string, turn: number, key: string, val: string) => {
					localStore[key] = val;
				},
			);
		...
		it("allows send_message to other agent types even after 4 turns", () => {
			...
			const localStore: Record<string, string> = {};
			coreMocks.getHookState.mockImplementation(
				(convId: string, turnIdx: number, key: string) => {
					return localStore[key] || null;
				},
			);
			coreMocks.setHookState.mockImplementation(
				(convId: string, turnIdx: number, key: string, val: string) => {
					localStore[key] = val;
				},
			);
	...
	describe("Phase 74 enhancements", () => {
		describe("remora-recall whitelist bypass", () => {
			it("allows remora-recall.ts query command for main agent even when matching rotPattern", () => {
				...
				const cmd =
					"node packages/adapter-antigravity/bin/remora-recall.ts --query 'something'";
			...
			it("allows remora-recall query command for subagent and keeps readonly check", () => {
				...
				// Allowed recall call
				coreMocks.inspectCommand.mockReturnValue(["allow", ""]);
				const ctx1 = makeCtx("run_command", {
					CommandLine: "remora-recall --query 'test'",
				});
				...
				const ctx2 = makeCtx("run_command", {
					CommandLine: "remora-recall --query 'test' && touch foo",
				});
				const res2 = main(ctx2);
				expect(res2["decision"]).toBe("deny");
		...
		describe("isBranch process.cwd() adaptation", () => {
			it("recognizes branch workspace when process.cwd() resolves to worktrees", () => {
				...
				process.cwd = () =>
					"/home/agent/.system_generated/worktrees/branch-123";
				mocks.realpathSync.mockReturnValue(
					"/home/agent/.system_generated/worktrees/branch-123",
				);
	...
	describe("unified watermark checks", () => {
		...
		it("allows view_file/grep_search on exempted paths even if limit is exceeded", () => {
			...
			const ctx1 = makeCtx("view_file", {
				AbsolutePath: "/path/to/any/file.json",
			});
			const res1 = main(ctx1);
			expect(res1["decision"]).toBe("allow");
