...
import { warn, error, debug, HOOKS_PROFILE_LOG, setTraceId } from "@remora/core";
...
export function hookEntrypoint(fallbackResult?: Record<string, unknown>) {
  ...
  return function (func: (inputData: Record<string, unknown>) => Record<string, unknown>) {
    return function (): void {
      ...
      try {
        ...
        ProgressSentinel.update(transcriptPath, "running", undefined, `Starting hook: ${hookName}`);

        const result = func(inputData);
        activeProfiler.step("func_execute");

        if (result && typeof result === "object" && result["decision"] === "deny") {
          ProgressSentinel.update(transcriptPath, "blocked", undefined, `Blocked by hook ${hookName}: ${result["reason"] ?? "No reason provided"}`);
        } else {
          let status = "running";
        ...
        const isToolUse = !!(inputData && typeof inputData === "object" && inputData["toolCall"] != null);
        const isStopHook = !!(inputData && typeof inputData === "object" && inputData["executionNum"] != null);
        const isInvocationHook = !!(inputData && typeof inputData === "object" && inputData["invocationNum"] != null);
        ...
        if (se instanceof SystemExit) {
          ...
          if (exitCode === 0) {
            ProgressSentinel.update(transcriptPath, "running", undefined, `Hook ${hookName} exited with code 0`);
            ...
            ProgressSentinel.update(transcriptPath, "blocked", undefined, `Hook SystemExit ${exitCode}`);
            if (isToolUse || isStopHook) {
              process.stdout.write(JSON.stringify({ decision: "deny", reason: `SystemExit with code ${exitCode}` }) + "\n");
            } else {
          ...
          if ("decision" in safeFallback) {
            safeFallback["decision_reason"] = `Remora Fallback (Error: ${String(se)})`;
          }
          error(`Hook Error: ${String(se)}`);
          process.stderr.write(se.stack ?? String(se) + "\n");
          ProgressSentinel.update(transcriptPath, "blocked", undefined, `Hook Exception: ${String(se)}`);
          if (isToolUse || isStopHook) {
          ...
          ProgressSentinel.update(transcriptPath, "blocked", undefined, `Hook Fatal Exception: ${String(se)}`);
          if (isToolUse || isStopHook) {
            process.stdout.write(JSON.stringify({ decision: "deny", reason: `Fatal Exception: ${String(se)}` }) + "\n");
          } else {
            process.stdout.write(JSON.stringify({}) + "\n");
          }
        }
