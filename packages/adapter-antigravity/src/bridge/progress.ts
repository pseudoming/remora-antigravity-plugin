...
export class ProgressSentinel {
    static getProgressPath(transcriptPath: string): string | null {
        ...
        if (path.basename(path.dirname(transcriptPath)) === ".system_generated") {
            return path.join(path.dirname(path.dirname(transcriptPath)), "scratch", "progress.json")
        } else {
            return path.join(path.dirname(transcriptPath), "scratch", "progress.json")
        }
    }

    static update(transcriptPath: string, status: string, stepIndex?: number, details: string = ""): boolean {
        if (!transcriptPath) {
            return false
        }
        ...
        if (finalStepIndex === undefined) {
            if ("step_index" in oldData) {
                ...
                if (match) {
                    ...
                    finalStepIndex = new ConversationDataAccessLayer(convId).getMaxStepIndex()
                }
            }
        }
        ...
        const baseName = lastDot > 0 ? progressPath.slice(0, lastDot) : progressPath
