...
export function main(): void {
  ...
  console.log("\n=== 关联架构决策召回 (通道 A 反向牵引) ===");
  const decisionsFts = dao.recallDecisionsByFts5Topic(projectUuid, convId, ftsKeyword);
  ...
  console.log("\n=== 直接匹配架构决策 (通道 B) ===");
  const likeDecisions = dao.recallDecisionsByLike(projectUuid, convId, ftsKeyword);
