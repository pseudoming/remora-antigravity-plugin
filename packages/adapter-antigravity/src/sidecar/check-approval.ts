...
const CONF_PATH = path.join(path.dirname(getDataDir()), "conf", "approval.json");
...
export function checkPlanApproval(projectUuid: string): void {
  // [P0] Plan approval detection window scan
  const tPlanChange = getPlanChangeTime(projectUuid);
  if (!tPlanChange) {
    return;
  }

  const userMessages = getUserMessagesAfter(tPlanChange, projectUuid);
  ...
  if (hasApproval) {
    const planContent = getPlanContent(projectUuid);
    ...
    enqueueEvent(projectUuid, "plan_approval_sync", JSON.stringify(payloadData));
    console.log(`[Remora] 探测到项目 ${projectUuid} 用户审批信号，已向事件队列抛入 plan_approval_sync。`);
  }
}
