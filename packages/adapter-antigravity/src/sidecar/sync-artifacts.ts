...
export function scanAndIngestArtifacts(context: Record<string, unknown>): void {
  ...
  const projectUuid = process.env["ANTIGRAVITY_PROJECT_ID"] || process.env["REMORA_PROJECT_ID"] || "unknown";
  ...
  for (const filename of targetFiles) {
    ...
    insertArtifactMessage(syncConvId, 999900 + targetFiles.indexOf(filename), filename, content, JSON.stringify(["artifact_topic"]));

    const convId = extractConvId((context["transcriptPath"] as string) ?? "");
    if (convId) {
      insertFileChange(projectUuid, convId, filename, "artifact");
    }
    upsertArtifactTopic(projectUuid, "artifact_topic", `Consolidated architecture decisions from ${filename}`);
