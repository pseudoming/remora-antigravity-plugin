...
const CONV_MARKER_FILE = path.join(
	getDataDir(),
...
export function getOrCreateConversation(prompt: string): string {
			...
			if (shouldRollover) {
				...
				try {
					...
					throw new AgentApiError(
						`Fail-Fast: send-message failed. Abandoning execution. Error: ${stderr}`,
					);
				}
			}
		}
	...
	try {
		...
		const stderr = e?.stderr?.toString() || String(e);
		throw new AgentApiError(
			`Fail-Fast: new-conversation failed. Abandoning execution. Error: ${stderr}`,
		);
	}
}

export function extractFactualBaseline(
	convId: string,
	startLine: number,
): [string[], string[]] {
...
export function processSessions(startTime: number): void {
	...
	try {
		...
		for (const session of activeSessions) {
			...
			if (isSub) {
				const { changedFiles, referencedFiles } = extractSubagentReport(
					session.conversationId,
				);
				if (changedFiles.length || referencedFiles.length) {
					const activeTopic = _getActiveTopic(session.projectUuid, conn);
					if (activeTopic) {
						const [assocJson, refJson] = getTopicFiles(
							session.projectUuid,
							activeTopic,
							conn,
						);
						const existingAssoc: Array<{ file: string; source: string }> =
							assocJson ? JSON.parse(assocJson) : [];
						const existingRef: Array<{ file: string; source: string }> = refJson
							? JSON.parse(refJson)
							: [];
			...
			const [baselineFiles, baselineActions] = extractFactualBaseline(
				session.conversationId,
				lastMsgId,
			);
			...
			try {
				...
				for (const t of data.topics || []) {
					...
					for (const d of decisions) {
						...
						if (
							decisionExists(session.projectUuid, topicId, decisionText, conn)
						) {
							continue;
						}
