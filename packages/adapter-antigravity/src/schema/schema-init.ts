...
function initDb(): void {
  ...
  try {
    ...
    try {
      ...
      db.exec("ALTER TABLE topic_decisions ADD COLUMN user_confirmed INTEGER DEFAULT 0");
    }
    try {
      db.prepare("SELECT injected_count FROM topic_decisions LIMIT 1").run();
    } catch {
      db.exec("ALTER TABLE topic_decisions ADD COLUMN injected_count INTEGER DEFAULT 0");
    }
    try {
    ...
    try {
      ...
      db.exec("ALTER TABLE watermarks ADD COLUMN last_msg_id INTEGER DEFAULT 0");
    }
  } finally {
    db.close();
  }
