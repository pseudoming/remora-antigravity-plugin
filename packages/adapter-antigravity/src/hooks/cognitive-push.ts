...
function _getActiveTopicAndDecisions(uuid: string): [string | null, Array<Record<string, unknown>>] {
...
function _handlePreInvocation(context: Record<string, unknown>, convId: string, currentTurnIdx: number): { injectSteps: Array<Record<string, unknown>> } {
  // 检查同回合内是否已经注入过会话重载提示
  ...
  if (!session || session.is_cold_start === 0) {  // session[2] = is_cold_start
...
function _checkLineCEnabled(): boolean {
  const configPath = path.join(path.dirname(getDataDir()), "conf", "features.json");
  try {
...
function _runLineC(context: Record<string, unknown>, convId: string, currentTurnIdx: number): Array<Record<string, unknown>> {
  ...
  const prompt = buildConflictDetectionPrompt(cleanMsg, candidates as import("@remora/core").ConflictCandidate[]);
...
function _handlePreToolUse(context: Record<string, unknown>, convId: string, currentTurnIdx: number): { decision?: string; reason?: string; injectSteps: Array<Record<string, unknown>> } {
  const toolName = (context["toolName"] as string) ?? "";
  if (!["write_to_file", "multi_replace_file_content", "replace_file_content"].includes(toolName)) {
    return { injectSteps: [] };
  }

  const toolArgs = (context["toolArgs"] ?? {}) as Record<string, unknown>;
  const targetFile = (toolArgs["TargetFile"] ?? toolArgs["AbsolutePath"] ?? "") as string;
  if (!targetFile) {
    return { injectSteps: [] };
  }
  ...
  // 检查目标文件是否是规划制品
  const isArtifact = isPlanningArtifact(targetFile,
    "/artifacts/",
...
export function main(context: Record<string, unknown>): { decision?: string; reason?: string; injectSteps: Array<Record<string, unknown>> } {
  try {
...
function _main(context: Record<string, unknown>): { decision?: string; reason?: string; injectSteps: Array<Record<string, unknown>> } {
  const fallback: { injectSteps: Array<Record<string, unknown>> } = { injectSteps: [] };
  ...
  try {
    if (stage === "pre-invoke") {
      return _handlePreInvocation(context, convId, currentTurnIdx) as { decision?: string; reason?: string; injectSteps: Array<Record<string, unknown>> };
    } else {
      // stage === "pre-tool"
