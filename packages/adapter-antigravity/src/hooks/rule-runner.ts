import { RuleEngine, Rule, Fact, IFactExtractor, DecisionResult } from "@remora/core";
  ...
  public extract(rawPayload: Record<string, any>): Fact {
    ...
    if (toolName === "invoke_subagent") {
      ...
      for (const sub of subagents) {
        ...
        if (tName === "Remora_Deep_Diver" && ws !== "branch" && ws !== "share") {
    ...
    const gitEscapeDetected = toolName === "run_command" && inspectCommand((args["CommandLine"] as string) ?? "")[0] !== "allow";
...
export class RuleRunner {
  ...
  public runActiveBlock(hookType: string, rawContext: Record<string, any>): DecisionResult {
    this.loadRules();
  ...
  public runDarkRead(hookType: string, rawContext: Record<string, any>): void {
    try {
      ...
      info(`[RuleRunner DarkRead] hookType: ${hookType}, result: ${JSON.stringify(result)}`);
    } catch (e: any) {
