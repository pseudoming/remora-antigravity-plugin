...
export function logDuration(elapsed: number, exitCode: number = 0): void {
	try {
		if (
			fs.existsSync(HOOKS_PROFILE_LOG) &&
			fs.statSync(HOOKS_PROFILE_LOG).size > 1024 * 1024
		) {
			fs.writeFileSync(
				HOOKS_PROFILE_LOG,
		...
		fs.appendFileSync(
			HOOKS_PROFILE_LOG,
			`=== [zombie-detector.ts] Run at ${new Date().toISOString()} ===\n`,
			"utf-8",
		);
		fs.appendFileSync(
			HOOKS_PROFILE_LOG,
...
function _main(context?: any): {
	...
	const transcriptPath =
		context && typeof context === "object"
			? (context.transcriptPath as string)
			: "";
	const match = transcriptPath
		? transcriptPath.match(/\/brain\/([^/]+)\//)
		: null;
	...
	for (const pid of pids) {
		...
		try {
			...
			const statData = fs
				.readFileSync(path.join(pidDir, "stat"), "utf-8")
				.split(/\s+/);
