...
export function main(context: Record<string, unknown>): Record<string, unknown> {
  ...
  try {
    ...
    console.error(`[PostFilter] Exception in executeDynamicRuleChain: ${e.message}`);
...
export type DynamicRule = (ctx: DynamicRuleContext) => Record<string, unknown> | undefined;

function buildDynamicRuleContext(rawContext: Record<string, unknown>): DynamicRuleContext {
  const toolCall = rawContext["toolCall"] as Record<string, unknown> | undefined;
...
function trimTimelineRule(ctx: DynamicRuleContext): Record<string, unknown> | undefined {
  trimStaleHookStates(ctx.convId, ctx.currentTurnIdx);
  return undefined;
}

function checkDuplicateSpawnRule(ctx: DynamicRuleContext): Record<string, unknown> | undefined {
  ...
  if (Array.isArray(subagents)) {
    for (const req of subagents) {
      ...
      let historyStr = getHookState(ctx.convId, ctx.currentTurnIdx, "subagent_spawns");
      let history: any[] = [];
      ...
      setHookState(ctx.convId, ctx.currentTurnIdx, "subagent_spawns", JSON.stringify(newHistory));
    }
  }
  return undefined;
}

function checkPromptSyntaxRule(ctx: DynamicRuleContext): Record<string, unknown> | undefined {
  if (ctx.toolName !== "invoke_subagent") return undefined;
  const subagents = (ctx.args["Subagents"] as Array<Record<string, unknown>>) ?? [];
  const rawHistory = getHookState(ctx.convId, ctx.currentTurnIdx, "subagent_dispatch_history");
  ...
  setHookState(ctx.convId, ctx.currentTurnIdx, "subagent_dispatch_history", JSON.stringify(history));
  return undefined;
}

function injectSubagentJITRule(ctx: DynamicRuleContext): Record<string, unknown> | undefined {
  if (ctx.toolName !== "invoke_subagent") return undefined;
  const jitInjected = getHookState(ctx.convId, ctx.currentTurnIdx, "subagent_jit");
  if (!jitInjected) {
...
function checkDefineSubagentOverrideRule(ctx: DynamicRuleContext): Record<string, unknown> | undefined {
...
function checkSharedWorkspaceTraversalRule(ctx: DynamicRuleContext): Record<string, unknown> | undefined {
...
function checkSendMessageTurnLimitRule(ctx: DynamicRuleContext): Record<string, unknown> | undefined {
  ...
  if (recipient) {
    ...
    const currentCount = parseInt(getHookState(ctx.convId, 0, stateKey) || "0", 10);
    setHookState(ctx.convId, 0, stateKey, String(currentCount + 1));
  }
  return undefined;
}

function checkUnifiedReadLimitRule(ctx: DynamicRuleContext): Record<string, unknown> | undefined {
  if (ctx.toolName === "view_file") {
    ...
    if (targetFile) {
      ...
      if (!ctx.isSub && ctx.transcriptPath) {
        ...
        if (incBytes > 0) {
          try {
            let statsAccumulated = accumulate(ctx.convId, isDataLog ? 0 : incBytes, isDataLog ? incBytes : 0, incBytes);
    ...
    if (searchPath) {
      ...
      if (grepBytes > 0) {
        try { accumulate(ctx.convId, 0, 0, grepBytes); } catch {}
      }
    }
  }
  return undefined;
...
function checkGitMcpRule(ctx: DynamicRuleContext): Record<string, unknown> | undefined {
  const isLazyMcpMatch = ctx.toolName === "call_mcp_tool" && ((ctx.args["ServerName"] as string) || "").replace(/_/g, "-") === "remora-git-mcp";
  const isEagerMcpMatch = ctx.toolName.startsWith("mcp_") && /^mcp_remora[-_]git[-_]mcp_/i.test(ctx.toolName);
...
function checkRunCommandAuditRule(ctx: DynamicRuleContext): Record<string, unknown> | undefined {
  ...
  const rotPattern = /\b(?:cat|tail|grep|jq|awk|sed|sqlite3)\b.*?(?:\.jsonl|\.log|\.sqlite)\b|\bremora-recall\.(?:py|ts)\b/i;
  ...
  if (hasRotFeature) {
    ...
    if (decision === "deny") {
      ...
      if (!ctx.isSub) {
        const blastDone = getHookState(ctx.convId, ctx.currentTurnIdx, "blast_radius");
        if (!blastDone) {
...
export function executeDynamicRuleChain(rawContext: Record<string, unknown>): Record<string, unknown> | undefined {
