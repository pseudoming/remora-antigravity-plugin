...
function walkFiles(
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            ...
            result.push(...walkFiles(fullPath, extensions, generatedFiles, pluginRoot));
        } else if (entry.isFile()) {
            if (generatedFiles.has(entry.name) || entry.name === "ORIGINAL_REQUEST.md") {
                continue;
            }
            if (relPath.startsWith("agents" + path.sep) && entry.name.endsWith(".json") && !entry.name.endsWith(".template.json")) {
                continue;
            }
...
describe("QualityGate", () => {
    ...
    function getAllFiles(extensions: string[]): string[] {
        const generatedFiles = new Set(["hooks.json", "sidecar.json", "SKILL.md", "mcp_config.json"]);
    ...
    it("no hardcoded absolute gemini paths", () => {
        const badHomePattern = new RegExp("~/\\.gemini/(?!antigravity|config/)", "g");
        const badAbsPattern = new RegExp("/home/[^/]+/\\.gemini/(?!antigravity)", "g");
    ...
    it("no arbitrary python subprocess", () => {
        const badPython3 = new RegExp("subprocess\\.(?:run|Popen|call)\\(\\s*\\[?\\s*['\"]python3['\"]", "g");
