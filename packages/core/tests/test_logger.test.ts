...
beforeEach(() => {
  vi.resetModules();
  tempDir = path.join(os.tmpdir(), `remora-logger-test-${randomUUID().slice(0, 8)}`);
...
describe("TestRotation", () => {
  it("rotation renames yesterdays log", async () => {
    ...
    if (fs.existsSync(current)) {
      expect(fs.statSync(current).mtime.getTime()).not.toBe(yesterday.getTime());
    }
...
describe("TestCleanup", () => {
  it("cleanup removes files older than 3 days", async () => {
    ...
    for (const daysAgo of [4, 5, 6]) {
      const dateStr = new Date(now - daysAgo * 86400_000).toISOString().slice(0, 10);
