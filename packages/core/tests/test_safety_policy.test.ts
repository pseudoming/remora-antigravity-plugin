...
describe("enforcePromptLengthLimit", () => {
	...
	it("should deny when stripped prompt exceeds maxChars", () => {
		...
		expect(reason!.message).toBe(
			"Subagent Prompt stripped length (1600 chars) exceeds 1500 limit. (Raw length: 1616 chars)",
		);
	});
...
describe("estimateReadBytes", () => {
	...
	beforeAll(() => {
		tmpFileWithLines = join(
			tmpdir(),
			`test-estimate-read-bytes-lines-${Date.now()}.txt`,
		);
		writeFileSync(tmpFileWithLines, "dummy content line\n");

		tmpFileSized = join(
			tmpdir(),
			`test-estimate-read-bytes-sized-${Date.now()}.dat`,
		);
	...
	afterAll(() => {
		try {
			unlinkSync(tmpFileWithLines);
		} catch {}
		try {
			unlinkSync(tmpFileSized);
		} catch {}
	});
...
describe("validatePromptSyntax", () => {
	it("should pass valid prompts", () => {
		const prompt =
			"This is a valid prompt without any unclosed brackets or quotes.";
	...
	it("should pass prompts with properly closed XML tags", () => {
		const prompt =
			"<system-reminder>This is a warning</system-reminder> and <system-discipline>Follow rules</system-discipline>";
		const result = validatePromptSyntax(prompt);
		expect(result.isValid).toBe(true);
	});
	...
	it("should fail on unclosed system-reminder tags", () => {
		...
		expect(result.errorReason).toContain(
			"XML tag '<system-reminder>' is not properly closed",
		);
	});
	...
	it("should fail on unclosed system-discipline tags", () => {
		...
		expect(result.isValid).toBe(false);
		expect(result.errorReason).toContain(
			"XML tag '<system-discipline>' is not properly closed",
		);
	});
	...
	it("should fail on unbalanced brackets outside code blocks", () => {
		const prompt =
			'This has a closed parenthesis (like this) and double quotes "but nothing else.';
		const result = validatePromptSyntax(prompt);
		expect(result.isValid).toBe(false);
...
describe("unified safety features", () => {
	...
	afterAll(() => {
		try {
			unlinkSync(tmpFileSmall);
		} catch {}
		try {
			unlinkSync(tmpFileLarge);
		} catch {}
	});
