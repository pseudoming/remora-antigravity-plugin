...
export function decodeBase64Token(token: string): string | null {
	if (token.length > 16 && /^[A-Za-z0-9+/]+={0,2}$/.test(token)) {
		try {
...
function inspectTokens(
	...
	for (let sub of subCommands) {
		...
		// 提取环境变量赋值
		while (sub.length > 0 && sub[0].includes("=") && !sub[0].startsWith("-")) {
		...
		if (exe === "grep") {
			for (const flag of args) {
				if (
					flag.startsWith("-") &&
					(flag.includes("r") || flag.includes("R"))
				) {
					return ["deny", "test"];
				}
			}
		...
		if (
			exe === "mvn" &&
			(args.includes("package") || args.includes("install"))
		) {
			return ["deny", "build"];
		}
	}
